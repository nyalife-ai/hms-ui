        ...
        <Table headers={["When", "Medication", "Batch", "Type", "Qty", "Ref", "By", "Notes"]}>
          {rows.map((m) => (
            <tr key={m.id} className="hover:bg-surface-200/60">
              ...
              <td className="px-5 py-3.5 font-medium text-foreground">{m.medicationName}</td>
              <td className="px-5 py-3.5 text-foreground-light">{m.batchNumber}</td>
              <td className="px-5 py-3.5">
                <Badge tone={m.quantityChange < 0 ? "amber" : "green"}>{m.movementType}</Badge>
              </td>
              <td className="px-5 py-3.5 text-foreground-light">{m.quantityChange}</td>
              <td className="px-5 py-3.5 text-xs text-foreground-light">{m.referenceType || "—"}</td>
              <td className="px-5 py-3.5 text-xs text-foreground-light">{m.performedByName || "—"}</td>
              <td className="px-5 py-3.5 text-xs text-foreground-light">{m.notes || "—"}</td>
            </tr>
          ))}
        </Table>
        {rows.length === 0 && !loading && (
          <p className="px-5 pb-5 text-sm text-foreground-lighter">No movements recorded yet.</p>
        )}
      </Card>
    </RoleGuard>
  );
